...
const getvideo = async () => {
    ...
    try {
        ...
        const URL = `https://api.adultdatalink.com/xhamster/video-information?video_url=${userInput}`;
        ...
        console.log(data);
        var img_url = data.thumbnails[10] // Set the image source to the thumbnail URL
        ...
        low.addEventListener('click', () => {
            open(data.video_qualities.h264["144p"], '_blank'); // Open the video URL in a new tab when the button is clicked
            //save the video URL in a new tab when the button is clicked

        })
        let videoUrl = data.video_qualities.h264;
        console.log(videoUrl); // Log the video URL object
        high.addEventListener('click', () => {
            open(data.video_qualities.h264["720p"], '_blank'); // Open the video URL in a new tab when the button is clicked
        });
    } catch (error) {
